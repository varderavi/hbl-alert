...
import threading
import os
import difflib  # 🎯 સ્પેલિંગ ચેક કરવા અને ઓટો-કરેક્ટ કરવા માટેની મેઈન લાઈબ્રેરી
...
# 🎯 મોસ્ટ પોપ્યુલર સ્ટોક્સનું માસ્ટર લિસ્ટ (જેમાંથી બોટ આપોઆપ સાચું નામ શોધી લેશે)
POPULAR_STOCKS = {
...
# ============================================
# 🎯 SMART AUTO-CORRECT & SUGGESTION LOGIC
# ============================================
def handle_search_text(user_text):
    query = user_text.upper().strip()
    
    # ૧. જો ડાયરેક્ટ મેપિંગ મળી જાય
    ...
    # ૨. 🧠 SPARK FUZZY MATCHING (સ્પેલિંગની ભૂલ પકડવી)
    all_keys = list(POPULAR_STOCKS.keys())
    # 80% મેચ થતા નજીકના સાચા નામો શોધશે
    close_matches = difflib.get_close_matches(query, all_keys, n=2, cutoff=0.6)
    
    if close_matches:
        # સ્માર્ટ સજેશન બટન તૈયાર થશે
        buttons = []
        ...
        send_telegram_msg(f"🤔 <b>ખોટો સ્પેલિંગ પકડાયો!</b>\n\nશું તમે આમાંથી કંઈક સર્ચ કરવા માંગો છો? નીચે ક્લિક કરો:", reply_markup=markup)
        return

    # ૩. જો લિસ્ટમાં ન હોય, તો આપમેળે .NS લગાવીને ડાયરેક્ટ ચેક કરશે
    symbol = f"{query}.NS"
    text, markup = generate_advanced_report(symbol, "5m")
    if text:
        send_telegram_msg(text, reply_markup=markup)
    else:
        # જો યાહુ ફાઇનાન્સ પર પણ સ્ટોક ન મળે તો છેલ્લો જવાબ
        fallback_markup = {"inline_keyboard": [[{"text": "🔙 Main Menu", "callback_data": "go_main"}]]}
        send_telegram_msg(f"❌ <b>સ્ટોક ન મળ્યો!</b>\n\n'<b>{query}</b>' નામનો કોઈ સ્ટોક ઇન્ડિયન માર્કેટમાં મળ્યો નથી. કૃપા કરીને સાચો સિમ્બોલ (e.g. WIPRO, TCS) લખો.", reply_markup=fallback_markup)

# ============================================
# TELEGRAM APP HANDLERS & WEB SERVER
# ============================================
...
def handle_callback(callback_id, data):
    ...
    if data == "m_hbl": text, markup = generate_advanced_report("HBLENGINE.NS", "5m")
        ...
        send_telegram_msg("🔍 <b>Pro Script Search Activated:</b>\n\nકૃપા કરીને નામ મોકલો (e.g. WIPR કે HBLL લખીને પણ ટેસ્ટ કરી શકો છો):")
        requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/answerCallbackQuery", json={"callback_query_id": callback_id})
        return
